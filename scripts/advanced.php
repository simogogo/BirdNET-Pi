...
if (isset($_GET["max_files_species"])) {
  ...
  if(isset($_GET["generate_ldfcs_standard"])) {
    if(strcmp(1,$config['GENERATE_LDFCS_STANDARD']) !== 0) {
      $contents = preg_replace("/GENERATE_LDFCS_STANDARD=.*/", "GENERATE_LDFCS_STANDARD=1", $contents);
    }
  } else {
    $contents = preg_replace("/GENERATE_LDFCS_STANDARD=.*/", "GENERATE_LDFCS_STANDARD=0", $contents);
  }

  if(isset($_GET["generate_ldfcs_indices"])) {
    if(strcmp(1,$config['GENERATE_LDFCS_INDICES']) !== 0) {
      $contents = preg_replace("/GENERATE_LDFCS_INDICES=.*/", "GENERATE_LDFCS_INDICES=1", $contents);
    }
  } else {
    $contents = preg_replace("/GENERATE_LDFCS_INDICES=.*/", "GENERATE_LDFCS_INDICES=0", $contents);
  }
...
?>
      ...
      <label for="generate_ldfcs_standard">LDFCS (Standard Thermal): </label>
      <input type="checkbox" name="generate_ldfcs_standard" <?php if($newconfig['GENERATE_LDFCS_STANDARD'] == 1) { echo "checked"; };?> >
      <p>Generate a daily Long-duration False-color Spectrogram (Standard magnitude with Magma colormap).</p><br>

      <label for="generate_ldfcs_indices">LDFCS (Acoustic Indices): </label>
      <input type="checkbox" name="generate_ldfcs_indices" <?php if($newconfig['GENERATE_LDFCS_INDICES'] == 1) { echo "checked"; };?> >
      <p>Generate a daily Long-duration False-color Spectrogram (Acoustic Indices mapped to RGB channels).</p><br>
